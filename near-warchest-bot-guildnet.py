...
def getEpochProgress():
    latestBlockHeight = int(runCommand("curl --silent https://rpc.openshards.io/status | jq .sync_info.latest_block_height")["output"])
    startHeight = int(runCommand("curl --silent -d '{\"jsonrpc\": \"2.0\", \"method\": \"validators\", \"id\": \"dontcare\", \"params\": [null]}' -H 'Content-Type: application/json' https://rpc.openshards.io | jq .result.epoch_start_height")["output"])
...
def getKickedOutReason():
    return os.popen("curl --silent -d '{\"jsonrpc\": \"2.0\", \"method\": \"validators\", \"id\": \"dontcare\", \"params\": [null]}' -H 'Content-Type: application/json' https://rpc.openshards.io | jq -c '.result.prev_epoch_kickout[] | select(.account_id | contains (\"" + poolId + "\"))' | jq .reason").read()
